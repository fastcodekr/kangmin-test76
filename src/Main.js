...
function Main() {
  ...
  useEffect(() => {
    const fetchUrl = async () => {
      await axios
        .get(BASE_URL + "/storage/url")
        .then((res) => setCurrentUrl(res.data.returnVal));
    };
    fetchUrl();
  }, [currentUrl]);
  ...
  const objectDelete = async (objectName) => {
    ...
    if (objectName) {
      ...
      if (
        window.confirm(`Are you sure you want to delete " ${objectName} " ?`)
      ) {
        try {
  ...
  const handleListObject = async (bucketName) => {
    try {
      if (bucketName) {
        setIsLoading(true);
        await axios.get(BASE_URL + "/bucket/listObject", {
            params: { bucketName },
          })
          .then((res) => {
  ...
  const handleBucketProperties = async (bucketName) => {
    if (bucketName) {
      await axios
        .get(BASE_URL + "/bucket/totalSize", { params: { bucketName } })
        .then((res) => {
  ...
  const handleBucketHeaders = async (bucketName) => {
    if (bucketName) {
      await axios
        .get(BASE_URL + "/bucket/header", { params: { bucketName } })
        .then((res) => {
  ...
  const handleBucketVersions = async (bucketName) => {
    if (bucketName) {
      await axios
        .get(BASE_URL + "/bucket/version", { params: { bucketName } })
        .then((res) => {
  ...
  const handleBucketPermission = async (bucketName) => {
    ...
    try {
      await axios
        .get(BASE_URL + "/bucket/permission", { params: { bucketName } })
        .then((res) => {
